...
globals().update(
    NotificationType.__members__
)  # Statically import the NotificationType enum
...
class Edupage:
    def __init__(
        self, school, username, password, timeout=5, log_level: LogLevel = LogLevel.NONE
    ):
        self.school = school
    ...
    def login(self):
        ...
        try:
            ...
            self.logger.warning(
                "There was a unhandled error, parsing login data. Printing additional information"
            )
            raise e
    ...
    def send_keepalive(self):
        ...
        response = self.session.post(
            f"https://{self.school}.edupage.org/login/eauth?portalping"
        )
    ...
    def parse_login_data(self, data):
        ...
        js_json = (
            data.split("$j(document).ready(function() {")[1]
            .split("});")[0]
            .replace("\t", "")
            .split("userhome(")[1]
            .replace("\n", "")
    ...
    def get_timetable(self, date):
        ...
        for subj in plan:
            ...
            if teacher_id is not None and len(teacher_id) != 0:
                ...
                self.logger.warning(
                    "The teacher's name couldn't be found in data or is none!"
                )
            ...
            if classroom_id is not None and len(classroom_id) != 0:
                ...
                self.logger.warning(
                    "The classroom's number couldn't be found in data or is none!"
                )
    ...
    def get_homework(self):
        ...
        if items is None:
            self.logger.warning(
                "The items entry in data is empty, couldn't parse homework!"
            )
            return None
        ...
        for item in items:
            ...
            if data is None:
                self.logger.warning(
                    "Found item with homework type with empty data, skipping..."
                )
                continue

            if data.get("triedaid") is None:
                self.logger.warning(
                    "Found item with homework type with empty classroom id, skipping..."
                )
                continue
    ...
    def get_news(self):
        ...
        if items is None:
            self.logger.warning(
                "The items entry in data is empty, couldn't parse news!"
            )
            return None
    ...
    def get_notifications(self):
        ...
        for notification in self.data.get("items"):
            ...
            date_added = notification.get("cas_pridania_btc")
    ...
    def get_lunch(self, date):
        ...
        try:
            ...
            lunch_data = json.loads(
                response.split("var novyListok = ")[1].split(";")[0]
            )
            self.logger.debug(
                f"Successfully parsed data and boarder id ({boarder_id})!"
            )
        except:
    ...
    def send_message(self, recipients, body, attachments=None):
        ...
        self.session.post(
            f"https://{self.school}.edupage.org/timeline/",
            headers=headers,
            params=params,
